...
public class ColumnGeneration : MonoBehaviour
{
    ...
    public GameObject flatColumn;
    public GameObject gapColumn;
    ...
    private void FlatWorld(){
        for(int i = 0; i < width; i++){
            
            SpawnObject(flatColumn, new Vector2(i, height));
            
        }
    }

    private void GapWorld(){
        for(int i = Random.Range(minGapWidth, maxGapWidth) ; i < width; i = i + Random.Range(minGapWidth, maxGapWidth)){
            SpawnObject(gapColumn, new Vector2(i, height));
            if (i>width){
                break;
            }
        }
